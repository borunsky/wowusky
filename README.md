...
```
◆ wowusky v0.4.12    Browse  Installed  WeakAuras  Import  Log   ● TBC Anniversary ⚙
```
...
- **Modular layout** under `wowusky/core/`, `wowusky/providers/`,
  `wowusky/catalog/`. app.py is down from 5179 to 4692 lines as provider and core logic moves into wowusky/providers/ and wowusky/core/
...
pip install build
python -m build
pip install dist/wowusky-0.4.12-py3-none-any.whl
```
...
│   ├── __init__.py
│   ├── __main__.py
│   ├── app.py              ← Tk GUI + install orchestration (~4692 lines)
│   ├── core/
