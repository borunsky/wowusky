...
### Fixed
- `wowusky` launcher entry point (`console_scripts`) failed to start because
  the `main()` dispatch function was nested inside `if __name__ == "__main__":`
  instead of being defined at module level. The app could not be launched after
  installation via pip / AUR. (#hotfix)
